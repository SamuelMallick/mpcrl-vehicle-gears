    ...
    def get_action(self, state: np.ndarray) -> tuple[float, float, int, dict]:
        ...
        if self.prev_sol:  # get gears from network for non-first time steps
            ...
            network_gear_choice_binary, network_action = (
                self.get_binary_gear_choice_from_network(nn_state)
            )
        ...
        if (
            self.use_heuristic
            or not self.prev_sol
            or (self.prev_sol and not sol.success)
        ...
        if (
            ...
            heuristic_flag = True
            sol = heuristic_sol
            gear_choice_explicit = np.argmax(heurisitic_gear_choice_binary, axis=0)
        else:
            gear_choice_explicit = np.argmax(network_gear_choice_binary, axis=0)
    ...
    def get_binary_gear_choice_from_network(
        ...
        if self.n_actions == 3:
            ...
            self.gear_choice_explicit = np.array(
                [
                    self.gear + np.sum(gear_shift[:, : i + 1])
